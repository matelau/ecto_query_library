defmodule FishingSpot.Data do
  ...
  defp _generate do
    ...
    mark = Repo.insert!(%Fisherman{name: "Mark", date_of_birth: format_date(~D[1970-01-01])})
    kirk = Repo.insert!(%Fisherman{name: "Kirk", date_of_birth: format_date(~D[1978-03-05])})
    joe = Repo.insert!(%Fisherman{name: "Joe", date_of_birth: format_date(~D[1973-10-15])})
    lew = Repo.insert!(%Fisherman{name: "Lew", date_of_birth: format_date(~D[1976-01-05])})

    fishermen = [mark, kirk, joe, lew]

    trip1 =
      Repo.insert!(%Trip{
        start_date: format_date(~D[2012-06-05]),
        end_date: format_date(~D[2012-06-12])
      })

    trip2 =
      Repo.insert!(%Trip{
        start_date: format_date(~D[2012-10-15]),
        end_date: format_date(~D[2012-10-20])
      })

    trip3 =
      Repo.insert!(%Trip{
        start_date: format_date(~D[2013-09-01]),
        end_date: format_date(~D[2013-09-02])
      })

    trip4 =
      Repo.insert!(%Trip{
        start_date: format_date(~D[2014-07-15]),
        end_date: format_date(~D[2014-07-20])
      })

    trip5 =
      Repo.insert!(%Trip{
        start_date: format_date(~D[2014-08-08]),
        end_date: format_date(~D[2014-08-18])
      })
    ...
    lake_fork =
      Repo.insert!(%Location{
        name: "Lake Fork",
        altitude: 8000,
        lat: Decimal.cast(38.090942),
        long: Decimal.cast(-107.292869),
    ...
    blue_mesa =
      Repo.insert!(%Location{
        name: "Blue Mesa",
        altitude: 7000,
        lat: Decimal.cast(38.472406),
        long: Decimal.cast(-107.210268),
    ...
    main_elk =
      Repo.insert!(%Location{
        name: "Main Elk Creek",
        altitude: 7000,
        lat: Decimal.cast(39.729579),
        long: Decimal.cast(-107.545462),
    ...
    frying_pan =
      Repo.insert!(%Location{
        ...
        altitude: 6000,
        lat: Decimal.cast(39.363448),
        long: Decimal.cast(-106.823121),
    ...
    unamed_pond =
      Repo.insert!(%Location{
        name: "Unamed Pond",
        altitude: 5000,
        lat: Decimal.cast(39.887929),
        long: Decimal.cast(-107.354998),
    ...
    white_river =
      Repo.insert!(%Location{
        ...
        altitude: 5000,
        lat: Decimal.cast(40.051879),
        long: Decimal.cast(-107.458016),
        location_type_id: pond.id
      })
    ...
    Enum.each(1..1000, fn _ ->
      ...
      Repo.insert(%FishLanded{
        date_and_time: attributes.date_and_time_caught,
        weight: attributes.weight,
        length: attributes.length,
        fisherman_id: attributes.fisherman.id,
        location_id: attributes.location.id,
        fly_type_id: attributes.fly.id,
        fish_species_id: attributes.fish.id
      })
    end)
    ...
    Repo.insert(%FishLanded{
      date_and_time: attributes.date_and_time_caught,
      weight: Decimal.new(:rand.uniform(50)),
      length: Decimal.new(:rand.uniform(100)),
      fisherman_id: mark.id,
      location_id: attributes.location.id,
      fly_type_id: attributes.fly.id,
      fish_species_id: attributes.fish.id
    })
    ...
    Repo.insert(
      Ecto.put_meta(%Account{identifier: "lew@example.com", name: "Lew"}, prefix: "users")
    )

    Repo.insert(
      Ecto.put_meta(%Account{identifier: "mark@example.com", name: "Mark"}, prefix: "users")
    )

    Repo.insert(
      Ecto.put_meta(%Account{identifier: "john@example.com", name: "John"}, prefix: "users")
    )
  end
  ...
  def get_fish_attributes(max_length, max_weight, fishermen, locations, flies, _trips, fish_types) do
    ...
    %RandomizedAttributes{
      ...
      length: length,
      weight: weight,
      date_and_time_caught: NaiveDateTime.truncate(date_and_time_caught, :second)
    }
    |> IO.inspect(label: "SSSS")
  end

  defp format_date(date) do
    {:ok, date_time} = NaiveDateTime.new(date, ~T[00:00:00])
    date_time |> NaiveDateTime.truncate(:second) |> NaiveDateTime.to_date()
  end
end
